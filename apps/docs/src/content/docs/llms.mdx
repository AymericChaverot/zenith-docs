...
Each page is also served as Markdown, at the same path with a `.md` extension: `/installation/` is also available at `/installation.md`. Every page links to its own Markdown version:
...
:::tip
Set `site` in your Astro configuration to get absolute URLs in the generated files, which is what most tools expect.
:::
