...
Override the families with three tokens, whatever the preset:
...
`--font-display` is used by `h1` and `h2` only, and falls back to `--font-body` when a preset gives headings no voice of their own. Set `fonts: false` to ship no font at all and use the system stack.
