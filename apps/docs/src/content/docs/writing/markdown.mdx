...
The six types are `note`, `tip`, `important`, `success`, `warning` and `danger`. Aliases keep their own title: `info` is a note, `idea` a tip, `warn` a warning, and `caution` and `error` are dangers.
