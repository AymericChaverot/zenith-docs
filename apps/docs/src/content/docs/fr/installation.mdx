...
Dans un terminal, la commande demande le dossier, le titre, le gabarit (`cli` ou `astro`), un [thème](/fr/customization/theming/#packaged-themes) et s'il faut installer les dépendances. Chaque réponse a son option, pour les scripts : `--template`, `--title`, `--theme` et `--install`, et `--yes` accepte les valeurs par défaut.
